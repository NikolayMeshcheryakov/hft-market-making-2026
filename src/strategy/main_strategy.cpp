                         ...
                         const std::vector<Trade>& trades)
{
    ...
    while (li < lob.size() || ti < trades.size())
    {
        ...
        if (process_lob)
        {
            ...
            if (om.bid_order && om.bid_order->active && market_ask > 0 && om.bid_order->price >= market_ask)
            {
            ...
            if (om.ask_order && om.ask_order->active && market_bid > 0 && om.ask_order->price <= market_bid)
            {
